...
mb_size = 20
z_dim = 1000
X_dim = input_data.WIDTH * input_data.HEIGHT * 3
y_dim = 549 #28
h_dim = 128
#h2_dim = 256
c = 0
...
def plot(samples):
    ...
    for i, sample in enumerate(samples):
        ...
        ax.set_yticklabels([])
        ax.set_aspect('equal')
        plt.imshow(sample.reshape(input_data.HEIGHT, input_data.WIDTH, -1)) #, cmap='Greys_r')
...
#Q_W4 = tf.Variable(xavier_init([h_dim, h2_dim]))
#Q_b4 = tf.Variable(tf.zeros(shape=[h2_dim]))

#Q_W5 = tf.Variable(xavier_init([h2_dim, h2_dim]))
#Q_b5 = tf.Variable(tf.zeros(shape=[h2_dim]))
...
#    h4 = tf.nn.relu(tf.matmul(h3, Q_W4) + Q_b4)
#    h5 = tf.nn.relu(tf.matmul(h4, Q_W5) + Q_b5)
...
#P_W6 = tf.Variable(xavier_init([h_dim, h2_dim]))
#P_b6 = tf.Variable(tf.zeros(shape=[h2_dim]))

#P_W7 = tf.Variable(xavier_init([h2_dim, h2_dim]))
#P_b7 = tf.Variable(tf.zeros(shape=[h2_dim]))

P_W8 = tf.Variable(xavier_init([h_dim, X_dim]))
P_b8 = tf.Variable(tf.zeros(shape=[X_dim]))
...
#    h6 = tf.nn.relu(tf.matmul(h5, P_W6) + P_b6)
#    h7 = tf.nn.relu(tf.matmul(h6, P_W7) + P_b7)
    logits = tf.matmul(h5, P_W8) + P_b8
...
with tf.Session() as sess:
    ...
    train_writer = tf.summary.FileWriter('\\tmp\\train_vae\\10', sess.graph)
    sess.run(tf.global_variables_initializer())

    saver = tf.train.Saver()
    saver.restore(sess, '\\tmp\\model\\cvae_model10.ckpt')
    ...
    for it in range(epochs):
        ...
        if it % (epochs/10) == 0:
            ...
            save_path = saver.save(sess, '\\tmp\\model\\cvae_model10.ckpt')
